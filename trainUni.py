...
# def mlp(X,y):
#     clf = MLPClassifier(hidden_layer_sizes=(128, 32), max_iter=250, learning_rate='adaptive', learning_rate_init=0.02)
#     clf.fit(X, y)
#     return clf
...
def mlp(X, y, X_test, y_test, modelName):
    ...
    model = Sequential()
    model.add(Dense(64, input_shape=(16*16,), activation='relu'))
    ...
    model.compile(
        tensorflow.keras.optimizers.Adam(lr=0.0005, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0),
